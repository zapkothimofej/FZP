...
export function StatsSection() {
  ...
  // Parse numeric value from stat string for count-up
  function parseStatValue(val: string): { prefix: string; num: number | null; suffix: string } {
    // "3" -> num=3
    // "100%" -> num=100, suffix="%"
    // "1 Partner" -> num=1, suffix=" Partner"
    // "∞" -> num=null
  ...
  useGSAP(
    () => {
      ...
      stats.forEach((stat, i) => {
        ...
        if (num === null) return // ∞ — no count up
        ...
        gsap.to(obj, {
          ...
          onUpdate: () => {
            el.textContent =
              prefix +
              (Number.isInteger(num) ? Math.round(obj.val).toString() : obj.val.toFixed(1)) +
              suffix
          },
        })
      })
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px border border-[#222222]">
          {stats.map((stat, i) => {
            ...
            return (
              <div
                key={stat.label}
                className="stat-card flex flex-col items-center justify-center py-16 px-4 text-center"
                style={{
