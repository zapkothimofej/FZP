...
export function PortfolioSection() {
  ...
  const handleMouseEnter = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    gsap.to(e.currentTarget.querySelector(".portfolio-inner"), {
      scale: 1.03,
      duration: 0.4,
      ease: "power2.out",
    })
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-3 gap-px bg-[#222222]">
        {portfolioPlaceholders.map((item) => (
          <div
            key={item.id}
            className={`portfolio-card cursor-pointer overflow-hidden ${getSizeClasses(item.size)} ${getHeight(item.size)}`}
            style={{ opacity: 0 }}
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
          >
            <div
              className="portfolio-inner w-full h-full flex flex-col justify-end p-8 relative"
              style={{
